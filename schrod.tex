...
The simplest possible wave function is the planar sine wave. 
...
\[
\psi(x, y, z, t) = \exp\left( 2\pi i \left[ \frac{x}{\lambda} - \frac{t}{\tau} \right] \right)
\]

\noindent in which the spatial periodicity of the wave (or wavelength) is $\lambda$
and the temporal periodicity (or period) is $\tau$.
It can be checked that $\psi(x+\lambda, t)=\psi(x, t)$ and $\psi(x, t+\tau)=\psi(x,t)$.
The minus sign in the expression of the elementary wave function indicates a propagation along the $+x$ axis.

How can be related the wave parameters $\lambda$ and $\tau$ to the mechanical behavior of a particle of mass $m$?
...
\[
\lambda = \frac{h}{p} \quad \mbox{or} \quad p = \frac{h}{\lambda}
\]
...
The time periodicity defined by the period $\tau$ is related to the frequency $\nu = 1/\tau$. 
...
\[
E = h\nu \quad \mbox{or} \quad E = \frac{h}{\tau}
\]
...
\[
\psi(x, y, z, t) = \exp\left(i \left[ x \frac{2\pi}{\lambda} - t \frac{2\pi}{\tau} \right] \right)
\]

\noindent that reduces to

\[
\psi(x, y, z, t) = \exp\left(i \left[ k x - \omega t \right] \right) \quad \mbox{with} \quad k=\frac{2\pi}{\lambda} \quad \mbox{and} \quad \omega=\frac{2\pi}{\tau}
\]

With the reduced Planck constant $\hbar = h/2\pi$

\[
E = \hbar \omega \quad \mbox{and} \quad p = \hbar k
\]
...
Therefore its total energy is the sum of its kinetic energy $T$ and of a potential energy $V$ so that
...
kinetic and potential energies may inter-convert.
...
Multiplying by $\psi$ the two terms of the classical expression of the total energy yields:
